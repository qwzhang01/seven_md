...
export function registerAllCommands() {
  ...
  const allCommands = [
    ...
    { id: 'view.editor', category: 'view' as const, title: '仅编辑器', icon: 'Code2', execute: () => { useUIStore.getState().setViewMode('editor-only') } },
    { id: 'view.preview', category: 'view' as const, title: '仅预览', icon: 'Eye', execute: () => { useUIStore.getState().setViewMode('preview-only') } },
    { id: 'view.sidebar', category: 'view' as const, title: '切换侧边栏', shortcut: 'Ctrl+B', icon: 'PanelLeft', execute: () => { useUIStore.getState().toggleSidebar() } },
    { id: 'view.zoomIn', category: 'view' as const, title: '放大', shortcut: 'Ctrl++', icon: 'ZoomIn', execute: () => { useUIStore.getState().zoomIn() } },
    ...
    { id: 'theme.light', category: 'theme' as const, title: '切换到浅色模式', icon: 'Sun', execute: () => { useThemeStore.getState().setTheme('light') } },
    { id: 'theme.monokai', category: 'theme' as const, title: 'Monokai', icon: 'Palette', execute: () => { useThemeStore.getState().setTheme('monokai') } },
    { id: 'theme.solarized', category: 'theme' as const, title: 'Solarized', icon: 'Palette', execute: () => { useThemeStore.getState().setTheme('solarized') } },
    { id: 'theme.nord', category: 'theme' as const, title: 'Nord', icon: 'Palette', execute: () => { useThemeStore.getState().setTheme('nord') } },
    { id: 'theme.dracula', category: 'theme' as const, title: 'Dracula', icon: 'Palette', execute: () => { useThemeStore.getState().setTheme('dracula') } },
    { id: 'theme.github', category: 'theme' as const, title: 'GitHub', icon: 'Palette', execute: () => { useThemeStore.getState().setTheme('github') } },
