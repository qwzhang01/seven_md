...
The system SHALL display tabs for all currently opened files at the **bottom** of the editor area, with enhanced interaction features.

#### Scenario: Tab bar is positioned at bottom of window
- **WHEN** the application window is displayed
- **THEN** a tab bar SHALL be visible at the bottom of the window
- **AND** the tab bar SHALL be positioned below all editor and preview content
...
### Requirement: Tab bar displays open file tabs (top position)
**Reason**: TabBar 位置已从顶部移动到窗口底部，改为 bottom-tab-bar spec
**Migration**: 使用新的底部标签栏功能
